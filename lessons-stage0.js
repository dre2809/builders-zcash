...
<p>Zcash uses the <strong>Digishield</strong> difficulty adjustment algorithm — faster and more responsive than Bitcoin's two-week adjustment period, preventing large swings in block time.</p>` },
    { title: "Block Rewards and the Dev Fund", body: `<p>When a miner finds a valid block, they receive the <strong>block reward</strong> — newly issued ZEC plus all transaction fees in the block. As of the Canopy network upgrade (2020), the block reward is split: <strong>80% goes to miners</strong>, and <strong>20% goes to the Zcash Dev Fund</strong> — distributed among ECC, the Zcash Foundation, and community grant recipients to fund ongoing protocol development.</p>
<p>Like Bitcoin, Zcash has a fixed supply cap of <strong>21 million ZEC</strong>, and the block reward halves approximately every four years. The halving creates predictable, diminishing supply over time.</p>` }
  ],
...
<p>The golden rule: <strong>build locally → test on testnet with TAZ → deploy to mainnet with real ZEC</strong>. Never skip the testnet step, especially for applications handling other people's funds.</p>` },
    { title: "Getting TAZ from ZecHub", body: `<p>The Zcash testnet faucet provides free TAZ for development. ZecHub's developer resources page at <code>zechub.wiki/using-zcash/testnet</code> lists current faucet URLs (they change periodically). Your wallet needs to be configured to connect to testnet nodes rather than mainnet nodes. Both Zebra and zcashd support testnet mode via configuration flags. Zaino also connects to testnet Zebra nodes when configured appropriately.</p>` }
  ],
