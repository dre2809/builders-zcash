...
"contribute-zaino": {
  ...
  sections: [
    { title: "What Zaino Needs", body: `<p>Zaino is younger than Zebra and has more surface area for contribution. Active areas: expanding gRPC API coverage, performance improvements to compact block delivery, address indexing, and expanding RPC coverage for legacy integration migration. The ZODL team is actively seeking contributors who have experience building applications on Zcash — you understand the gaps from the developer side.</p><p>Repo: <code>github.com/zodl-inc/zaino</code>. The codebase is Rust with tonic for gRPC. Get oriented by reading the architecture documentation at github.com/zodl-inc/zaino first.</p>` },
...
"contribute-zallet": {
  ...
  sections: [
    { title: "Zallet's Development Status", body: `<p>Zallet is under active development by ECC and is not yet at 1.0 — making it an excellent time to contribute. The codebase is Rust, targeting the Z3 stack. Contributions are especially valuable in: testing and validation of key management flows, RPC API coverage (implementing legacy-compatible methods for migration), documentation, and UI/UX for the daemon's JSON-RPC API surface.</p><p>Repo: <code>github.com/zodl-inc/zallet</code>. Check the project board for current priorities — ZODL maintains a public roadmap.</p>` },
    ...
    { title: "ZODL Contributor Guidelines", body: `<p>ZODL uses a signed Contributor License Agreement (CLA) — sign it before your first PR. They require all code to have tests, follow the existing code style (enforced by CI), and include clear commit messages. ZODL engineers do thorough code reviews focused on correctness and security. For wallet code where bugs can lose user funds, this rigor is appropriate — embrace it. The review process will make you a better engineer.</p>` }
  ],
