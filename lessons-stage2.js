...
"json-rpc": {
  ...
  sections: [
    ...
    { title: "Handling Async Operations", body: `<p>Some RPC operations are synchronous (block queries, balance checks). Others are long-running: <code>z_sendmany</code> may take several seconds due to proof generation. <strong>Note: zcashd is deprecated.</strong> If you are maintaining a legacy zcashd integration, it used async operation IDs — <code>z_sendmany</code> returned an opid, polled via <code>z_getoperationstatus</code>. For new integrations, use Zaino's gRPC <code>SendTransaction</code> — synchronous, typed, and actively maintained.</p>` },
...
"wallet-sdks": {
  ...
  sections: [
    ...
    { title: "SDK Best Practices", body: `<p>Both SDKs are production-ready and used in ECC's own wallets (Zodl). Key practices: always use a <strong>wallet birthday height</strong> when restoring from seed. Handle sync state explicitly in your UI (scanning, synced, error). Never block the main thread during proof generation. Store seeds securely using the platform's secure enclave / keychain. Implement proper backup and recovery flows — without seed backup, users can permanently lose funds.</p>` }
  ],
...
"run-zebra": {
  ...
  sections: [
    { title: "Installation", body: `<p>Install via Cargo: <code>cargo install --locked zebrad  # v4.5.3</code>, or download a pre-built binary from the GitHub releases page. Docker images are available: <code>docker pull zfnd/zebra</code>.</p><p>Minimum production hardware: 4 CPU cores, 16GB RAM, 200GB NVMe SSD (SSD is important — spinning disk is too slow for initial sync). A stable internet connection with at least 10Mbps is required for peer connectivity.</p>` },
