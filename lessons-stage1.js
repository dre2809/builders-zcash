...
"what-is-zcash": {
  ...
  sections: [
    ...
    { title: "Transparent and Shielded Addresses", body: `<p>Zcash supports two address types. <strong>Transparent addresses</strong> (starting with "t1") work like Bitcoin — all transaction details are public. <strong>Shielded addresses</strong> hide sender, receiver, and amount using ZK proofs.</p><p>The latest address type is <strong>Unified Addresses (UA)</strong> — a single address that can receive funds in any pool. The Zcash community and ZODL strongly encourage using shielded transactions by default.</p>` },
    { title: "Zcash vs Bitcoin", body: `<p>Zcash shares Bitcoin's fixed 21M supply cap, UTXO-based model, and proof-of-work consensus. Key differences: Zcash adds a shielded transaction pool, uses the Equihash PoW algorithm, had a 20% Dev Fund (now restructured post-NU6: 8% ZCG + 12% lockbox), and follows a faster halving schedule. For a developer, the most significant difference is that building on Zcash requires understanding the shielded pool's key management and proof generation.</p>` }
  ],
...
"zcashd": {
  stage: "Stage 01 · Understand Zcash", stageColor: "var(--gold)",
  title: "zcashd — Deprecated Node",
  subtitle: "zcashd has been officially deprecated and replaced by Zebra (zebrad). Understanding what it was and why it was sunset explains the architecture decisions behind the modern Z3 stack.",
  sections: [
    { title: "What zcashd Was (Historical)", body: `<p>zcashd was the reference Zcash node implementation, maintained by the Electric Coin Company (ECC) and forked from Bitcoin Core. It handled everything: peer-to-peer networking, consensus, mempool management, block production, wallet functionality, and a JSON-RPC API. One binary did it all.</p><p>For years, zcashd was the only way to run a Zcash node. Most exchanges, explorers, and wallet backends connected to zcashd via its RPC interface.</p>` },
    { title: "zcashd is Deprecated — Use Zebra", body: `<p>ECC announced in 2023 that zcashd would be deprecated. zcashd was officially deprecated in 2025. The reasons: the codebase was large and difficult to maintain, it bundled wallet and node concerns that are better separated, and a Rust-based alternative (Zebra) had matured to production-readiness. zcashd is being phased out in favor of the <strong>Z3 stack</strong> — Zebra + Zaino + Zallet — which cleanly separates consensus, data access, and wallet concerns.</p><p><strong>zcashd is no longer maintained.</strong> All new development must use the Z3 stack. If you are maintaining an existing zcashd integration, migrate immediately — zebrad is the active consensus node, Zaino is the active data layer, and Zingolib/Zallet handle wallet operations.</p>` },
    { title: "RPC Compatibility", body: `<p>Many legacy integrations (exchanges, payment processors) used zcashd's JSON-RPC API. Zebra implements a compatible subset of this API to ease migration. Check the Zebra documentation for the current RPC method coverage. For new integrations, always use Zaino's gRPC API — it is the supported, actively developed interface.</p>` }
  ],
...
"zallet": {
  stage: "Stage 01 · Understand Zcash", stageColor: "var(--gold)",
  title: "Zallet",
  subtitle: "Zallet is ZODL's wallet daemon — the team that continued Zcash core protocol development after departing ECC in January 2026. Modular, embeddable, and designed to be the wallet backend for the modern Z3 stack.",
  sections: [
    { title: "What Zallet Is", body: `<p>Zallet is a wallet library and daemon maintained by ZODL. Unlike the old monolithic zcashd (which bundled node and wallet together), Zallet is a <strong>separate, composable component</strong>. It connects to Zaino for chain data, handles key management and address derivation, manages transaction scanning and note tracking, and constructs and signs transactions — including generating zero-knowledge proofs for shielded transactions.</p><p>Zallet can be run as a standalone daemon (with a JSON-RPC API for local use) or embedded as a library in larger applications.</p>` },
    { title: "Zallet vs Zingolib", body: `<p>Both Zallet and Zingolib are Zcash wallet libraries with different origins. <strong>Zingolib</strong> (by Zingo Labs) is a mature, actively maintained library with production deployments in Ywallet and Zingo Wallet — currently has more production mileage. <strong>Zallet</strong> is ZODL's implementation, targeting the full Z3 stack. ZODL (Zcash Open Development Lab) was founded in January 2026 by the former ECC engineering team. Both are good choices for new development.</p>` },
...
"blocks": {
  ...
  sections: [
    { title: "Block Structure", body: `<p>A Zcash block contains: a <strong>block header</strong> (previous block hash, Merkle root of transactions, timestamp, difficulty target, nonce), a <strong>coinbase transaction</strong> (miner reward; 1.5625 ZEC post-May 2025 halving), and a list of regular transactions. Zcash blocks are produced approximately every 75 seconds — roughly 1152 blocks per day.</p>` },
...
"consensus-rules": {
  ...
  sections: [
    { title: "What Are Consensus Rules?", body: `<p>Consensus rules are the protocol-level checks that every node performs on every transaction and block it receives. Rules include: signatures must verify, notes must not be double-spent, ZK proofs must verify, transaction amounts must balance, block hash must meet difficulty target, and dozens of others.</p><p>Consensus rules are codified in the <strong>Zcash Protocol Specification</strong> — a formal document defining every rule precisely. Zebra implements these rules. zcashd, which also implemented them, has been deprecated.</p>` },
    { title: "Protocol Upgrades (Network Upgrades)", body: `<p>Zcash uses periodic network upgrades (NUs) to add features and change rules. Notable upgrades: <strong>Sapling (NU2, 2018)</strong> added fast shielded transactions. <strong>Canopy (NU4, 2020)</strong> activated the Dev Fund. <strong>NU6 (Nov 2025)</strong> restructured it: 8% ZCG grants + 12% community lockbox + 80% miners. <strong>NU5 (2022)</strong> added Orchard and Unified Addresses. Each upgrade activates at a specific block height. Applications must handle this correctly during transition periods.</p>` },
...
"rpc-basics": {
  stage: "Stage 01 · Understand Zcash", stageColor: "var(--gold)",
  title: "RPC Basics",
  subtitle: "JSON-RPC is the traditional interface for interacting with Zcash nodes. Zebra implements the most-used RPC methods for legacy compatibility, while new integrations should prefer Zaino's gRPC API.",
  sections: [
